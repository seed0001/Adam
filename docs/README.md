...
**A reasoning loop, not a single call.** Every message goes through `Classify → Plan → Execute → Observe`. The classifier outputs both **complexity** (trivial/simple/complex/multi-step) and **intent** (Brainstorming, Build, Research, Skill Development, General). Intent shapes how Adam responds: when you want to brainstorm an app, Adam focuses on ideation rather than jumping straight into building. Simple questions get fast direct answers. Complex requests get broken into a task DAG with dependency ordering. Tool calls happen within the execution phase, not as an afterthought.
...
| Web dashboard — chat, memory, status, settings, providers, scratchpad, skills, voices, diagnostics | ✅ |
...
| Voice synthesis (Edge, LuxTTS, XTTS) | ✅ |
...
Adam can create songs on [Suno](https://suno.com) via browser automation. Say "create a song about X" or "make me a chill lo-fi beat" and Adam will open Suno, enter your description, and click Create.

**First run:** When the browser opens, log in to Suno manually. Your session is saved to `~/.adam/suno_browser_data` for reuse.

**Sign-in:** Adam uses Google Chrome (or Microsoft Edge) for Suno — not Chromium — because Google/Suno block Chromium as "not secure." Install [Chrome](https://www.google.com/chrome/) or [Edge](https://www.microsoft.com/edge) if sign-in fails. Playwright: `npx playwright install chromium` (for Chromium fallback).
...
| **Voices** | Voice profiles for synthesis (Edge, Lux, XTTS) |
| **Diagnostics** | System diagnostics — codebase analysis, pipeline stages, run tests end-to-end |
...
└── packages/
    ...
    ├── core/            # Agent loop · Classifier · Planner · Executor · SkillWorkshop · ScratchpadStore · BuildSupervisor
...
## Voice (Edge, LuxTTS, XTTS)

Adam supports multiple voice providers:
- **Edge** — Microsoft Edge TTS (built-in voices)
- **LuxTTS** — [LuxTTS](https://huggingface.co/YatharthS/LuxTTS) via Python sidecar
- **XTTS** — Coqui XTTS for custom voice cloning

`adam init` can automate the LuxTTS sidecar installation and optionally start it automatically when voice is enabled.
